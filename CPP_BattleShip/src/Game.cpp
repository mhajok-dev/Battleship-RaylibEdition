...
Game::Game(const int screenWidth, const int screenHeight)
    : m_renderer(screenWidth, screenHeight), m_aiDelay(GameSettings::AI_DELAY)
{}
...
std::vector<int> Game::GetShipLengths() const
{
...
{
    const std::vector<int> shipLengths = GetShipLengths();
    ...
    for (const int length : shipLengths)
    {
        ...
        do
        {
            ...
            if (attempts > 1000)
            {
                ...
                board.GetGameBoard().assign(
                    m_boardSize, std::vector<char>(m_boardSize, '-')
                );
...
void Game::ProcessPlayerInput()
{
    ...
    const std::string move = std::to_string(col + 1) + static_cast<char>('A' + row);
    const MoveValidation validation = InputValidator::ValidateMove(move, m_boardSize, pAIBoard->GetHiddenBoard());

    if (!validation.m_bIsValid)
        return;

    pAIBoard->IncrementShotsFired();
...
{
    if (m_pAIPlayer->GetBoard()->GetWinCondition())
    {
        m_bPlayerWon = true;
        m_state = GameState::GAME_OVER;
    }
    else if (m_pHumanPlayer->GetBoard()->GetWinCondition())
    {
        m_bPlayerWon = false;
        m_state = GameState::GAME_OVER;
    }
...
void Game::ProcessAITurn(const float deltaTime)
{
    ...
    const std::string move = m_pAIPlayer->MakeMove();
    Board* pPlayerBoard = m_pHumanPlayer->GetBoard();

    const MoveValidation validation = InputValidator::ValidateMove(
        move, m_boardSize, pPlayerBoard->GetHiddenBoard()
    );

    if (!validation.m_bIsValid)
        return;

    pPlayerBoard->IncrementShotsFired();
    m_lastAIShot = { validation.m_row, validation.m_column };

    const ShotResult result = ApplyShot(*pPlayerBoard, validation.m_row, validation.m_column);
    const std::string coord = std::string(1, static_cast<char>('A' + validation.m_row))
                            + std::to_string(validation.m_column + 1);

    if (result == ShotResult::SUNK)
    {
        m_pAIPlayer->ClearTargetCells();
        m_audioManager.PlaySunk();
        m_lastStatusMessage = "AI shot at " + coord + " - Sunk!";
    }
    else if (result == ShotResult::HIT)
    {
        m_pAIPlayer->EnqueueAdjacentCells({ validation.m_row, validation.m_column });
        m_audioManager.PlayHit();
        m_lastStatusMessage = "AI shot at " + coord + " - Hit!";
    }
    else
    {
        m_audioManager.PlayMiss();
        m_lastStatusMessage = "AI shot at " + coord + " - Miss!";
    }
...
void Game::Draw() const
{
    ...
    {
        case GameState::MENU:
            m_renderer.DrawMenuScreen(m_boardSize, m_menuTimer);
            break;
